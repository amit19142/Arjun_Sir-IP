    ...
    for i in range(len(occupancy_date)):
        if type(occupancy_date[i]) == datetime.datetime:
            ...
            if index >= start and date_array[index] == occupancy_date[i]:
                rs_fit = sheet2.cell(row=index+1, column=9).value + sheet2.cell(row=index+1, column=10).value
                rs_groups = sheet2.cell(row=index+1, column=11).value + sheet2.cell(row=index+1, column=12).value
                rs_CH = sheet2.cell(row=index+1, column=6).value + sheet2.cell(row=index+1, column=7).value
